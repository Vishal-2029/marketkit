...
import { Check, Plus, Trash2, Pencil, Bold, Italic, Underline, List, ListOrdered } from "lucide-react";
...
export default function MarketPlansPage() {
  ...
  // modal state: null = closed, "create" = new plan, MarketPlan = editing existing
  const [modal, setModal] = useState<null | "create" | MarketPlan>(null);
  const [form, setForm] = useState<MarketPlanPayload>(emptyForm());
  const [priceInput, setPriceInput] = useState("");
  const [deleteTarget, setDeleteTarget] = useState<MarketPlan | null>(null);
  ...
  const { data: plans = [], isLoading } = useQuery({
    queryKey: ["market-plans"],
    queryFn: marketPlansService.list,
  });

  const createMut = useMutation({
    mutationFn: (data: MarketPlanPayload) => marketPlansService.create(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["market-plans"] });
      setModal(null);
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: MarketPlanPayload }) =>
      marketPlansService.update(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["market-plans"] });
      setModal(null);
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => marketPlansService.delete(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["market-plans"] });
  ...
  const toggleActive = (plan: MarketPlan) => {
  ...
  const openEdit = (plan: MarketPlan) => {
    setForm({
      name: plan.name,
  ...
  const handleSave = () => {
    ...
    const discount = form.fee_discount_pct ?? 0;
    if (discount < 0 || discount > 100) {
      toast.error("Fee discount must be between 0 and 100.");
      return;
    }
    ...
    if (modal === "create") {
      ...
      updateMut.mutate({ id: (modal as MarketPlan).id, data: payload });
    }
  };

  const isPending = createMut.isPending || updateMut.isPending;

  const visiblePlans = (plans as MarketPlan[]).filter(p => showInactive || p.is_active);
          ...
          {visiblePlans.map(plan => {
            ...
            return (
              <div key={plan.id} className="rounded-2xl border border-border bg-card p-6 flex flex-col">
                ...
                {/* Perks — placeholder: FeeDiscountPct + FeaturedSeller only */}
                {(plan.fee_discount_pct > 0 || plan.featured_seller) && (
                  <div className="space-y-1.5 mb-4 mt-2">
                    {plan.fee_discount_pct > 0 && (
                      <div className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-success shrink-0" />
                        <span className="text-sm text-foreground">{plan.fee_discount_pct}% fee discount</span>
                      </div>
                    )}
                    {plan.featured_seller && (
                      <div className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-success shrink-0" />
                        <span className="text-sm text-foreground">Featured seller badge</span>
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="bg-card rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-section-title">
                {modal === "create" ? "New Market Plan" : "Edit Market Plan"}
              </h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  max={100}
                  placeholder="0"
                  value={form.fee_discount_pct ?? 0}
                  onChange={e => setForm(f => ({ ...f, fee_discount_pct: Number(e.target.value) }))}
                />
                <p className="text-xs text-muted-foreground mt-1">0–100; knocks off market fee percent for sellers</p>
              </div>

              <div>
                <label className="text-sm font-medium text-foreground mb-2 block">Perks</label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <button
                    type="button"
                    className={`w-10 h-6 rounded-full relative transition-colors duration-200 ${form.featured_seller ? "bg-primary" : "bg-muted"}`}
                    onClick={() => setForm(f => ({ ...f, featured_seller: !f.featured_seller }))}
                  >
                    <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${form.featured_seller ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                  <span className="text-sm text-foreground">Featured seller badge</span>
                </label>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40">
          <div className="bg-card rounded-2xl shadow-xl w-full max-w-sm p-6">
            <h2 className="text-section-title mb-2">Delete Market Plan</h2>
