...
func TestActivateMarketPlanSub_CreditsPlatformWalletExactlyOnce(t *testing.T) {
	testutil.WithTx(t, func() {
		...
		assert.Equal(t, plan.PriceInPaise, total, "must credit the full plan price exactly once")
	})
}
...
func TestHandleSubscribeMarketPlanWithWallet_CreditsFullAmount(t *testing.T) {
	testutil.WithTx(t, func() {
		...
		user := testutil.MustCreateUser(t, tx)
		require.NoError(t, tx.Model(&user).Update("wallet_balance_in_paise", int64(100000)).Error)
		plan := testutil.MustCreateMarketPlan(t, tx, 49900)
		...
		require.NoError(t, tx.Select("wallet_balance_in_paise").First(&buyerAfter, "id = ?", user.ID).Error)
		assert.Equal(t, int64(100000-49900), buyerAfter.WalletBalanceInPaise)

		var platformTotal int64
		require.NoError(t, tx.Model(&models.PlatformLedger{}).
			Where("source = ?", models.PlatformSourceMarketPlan).
			Select("COALESCE(SUM(amount_in_paise), 0)").Scan(&platformTotal).Error)
		assert.Equal(t, plan.PriceInPaise, platformTotal, "platform wallet gets the full plan price, not just a cut")
	})
}
